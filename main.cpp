...
void showWindows(MyImage m){
	//code below commmented out in order to remove hand background subtraction image from corner:
...
void myDrawContours(MyImage *m,HandGesture *hg, Mat *overlay, int fingerNum){
	...
	if(fingerNum<=4){
   	//	circle (*overlay, hg->bRect.tl(), 20, Scalar(0,0,255), -1, 8, 0);
  	}
...
void makeContours(MyImage *m, HandGesture* hg, Mat *overlay){
	Mat aBw;
	pyrUp(m->bw,m->bw);
	m->bw.copyTo(aBw);
...
//		approxPolyDP( Mat(hg->contours[hg->cIdx]), hg->contours[hg->cIdx], 11, true );
		...
		if(isHand){	
			hg->getFingerTips(m);
			int fingerSize = hg->fingerTips.size();
			hg->drawFingerTips(m, overlay);
			myDrawContours(m,hg,overlay,fingerSize);
		}
	}
}
...
int main(){
	...
	init(&m);		
	m.cap >>m.src;
	Mat overlay(m.src.rows, m.src.cols, CV_8UC3, Scalar(255,255,255));
	double alpha = 0.5;

	//size overaly according to m.src
	resize(overlay, m.src, m.src.size(), 0, 0, cv::INTER_CUBIC);
	//overlay = Mat::ones(m.src, m.src, CV_64F);
	...
	for(;;){
		...
		makeContours(&m, &hg, &overlay);
		hg.getFingerNumber(&m);

		//overlay m with another image to get drawing on video feed
		/*
		if(hg.fingerTips.size()>0 && count < 5){
			//for(int i = 0; i < hg.fingerTips.size(); i++){
				int x = hg.fingerTips[0].x;
				int y = hg.fingerTips[0].y;

				Vec3b color = overlay.at<Vec3b>(Point(x,y));
				color[0] = 255;
				color[1] = 0;
				color[2] = 0;
				overlay.at<Vec3b>(Point(x,y)) = color;
			//}
			//count++;
		}
		*/
		//hg.drawJokes(&overlay);
		
		
		addWeighted(overlay, alpha, m.src, 1 - alpha, 0, m.src);
		//m.src.copyTo(overlay);

		//continue with rest of showing image
		showWindows(m);
		out << m.src;
		//imwrite("./images/final_result.jpg",m.src);
    	if(cv::waitKey(60) == char('q')) break;
	}
	destroyAllWindows();
	out.release();
	m.cap.release();
