...
void HandGesture::drawFingerTips(MyImage *m, Mat *overlay){
	Point p;
	int k=0;
	...
	for(int i=0;i<fingerTips.size();i++){
		...
		putText(m->src,intToString(i),p-Point(0,30),fontFace, 1.2f,Scalar(200,200,200),2);
   		...
   		if(fingerTips.size()>=4){
   			//circle(*overlay, fingerTips[0], 20, Scalar(255,255,255), -1, 8, 0);
   			overlay->setTo(Scalar(255,255,255));
   		}else if(fingerTips.size()==2){
   			Point one = fingerTips[0];
   			Point two = fingerTips[1];
   			Point mid((one.x+two.x)/2, (one.y+two.y)/2);
   			//drawMarker(*overlay, mid, Scalar(0,0,255), MARKER_STAR, 20, 10, 8);
   			circle (*overlay, mid, 18, Scalar(0,0,255), -10, 8, 0);
  		}
   	 }
}

void HandGesture::drawJokes(Mat *overlay){
	Point p;
	int k=0;
	...
	for(int i=0;i<fingerTips.size();i++){
		...
		//Vec3b color = overlay->at<Vec3b>(p);
		//color[0] = 0;
		//color[1] = 0;
		//color[2] = 255;
		//overlay->at<Vec3b>(p) = color;
		//putText(m->src,intToString(i),p-Point(0,30),fontFace, 1.2f,Scalar(200,200,200),2);
   		//circle( m->src,p, 
   	}
